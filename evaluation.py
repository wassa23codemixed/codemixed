...
def read_mlec(filename):
    df = pd.read_csv(filename)
    df['target_list'] = df[['anger','anticipation','disgust','fear','joy','love','optimism','pessimism','sadness','surprise','trust']].values.tolist()
...
def f1_score_mcec(y_true, y_pred, class_label):
    tp = sum([1 for true_label, pred_label in zip(y_true, y_pred) if true_label == class_label and pred_label == class_label])
    fp = sum([1 for true_label, pred_label in zip(y_true, y_pred) if true_label != class_label and pred_label == class_label])
    fn = sum([1 for true_label, pred_label in zip(y_true, y_pred) if true_label == class_label and pred_label != class_label])
    precision = tp / (tp + fp) if (tp + fp) > 0 else 0
    recall = tp / (tp + fn) if (tp + fn) > 0 else 0
    f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0
    return f1
...
def score(args):
    ...
    goldList_mlec = read_mlec(os.path.join(args.gold_path, 'gold_mlec.csv'))
    goldList_mcec = read_mcec(os.path.join(args.gold_path, 'gold_mcec.csv'))

    task1, task2 = False, False
    if args.mlec_path:
        mlec_submission_path = os.path.join(args.mlec_path, 'predictions_MLEC.csv')
    ...
    if args.mcec_path:
        mcec_submission_path = os.path.join(args.mcec_path, 'predictions_MCEC.csv')
    ...
    jaccard, micro_f1, macro_f1, macro_f1_mcec, accuracy, precision, recall = calculate_metrics(goldList_mlec, goldList_mcec, predictedList_MLEC, predictedList_MCEC, task1, task2)

    with open(os.path.join('scores.txt'), 'w') as output_file:
        str_to_write = ''
        if task1:
            str_to_write += "Task1: Multi label accuracy: {0}\nMacro F1: {1}\nMicro F1: {2}\n".format(jaccard, micro_f1, macro_f1)
        if task2:
            str_to_write += "Task2: Macro F1-Score: {0}\nRecall: {1}\nPrecision: {2}\nAccuracy: {3}\n".format(macro_f1_mcec, recall, precision, accuracy)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-g', '--gold_path', default='gold')
    parser.add_argument('-l', '--mlec_path', default='test')
    parser.add_argument('-c', '--mcec_path', default='test')
